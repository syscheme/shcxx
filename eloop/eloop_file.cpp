...
void File::fs_event_cb(uv_fs_event_t *handle, const char *filename, int events, int status) {

	File* self = static_cast<File *>(handle->data);
	if (self != NULL) {
		self->OnFile_cb(self, filename, events, status);
	}
}
...
void Pipe::connect_cb(uv_connect_t *req, int status) {
	uv_stream_t* stream = req->handle;

	Pipe* self = static_cast<Pipe *>(stream->data);

	if (self != NULL) {
		self->OnPipeConnect_cb(self, status);
	}
