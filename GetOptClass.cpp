...
bool GetOpt::ParseCmdLine(const std::string& str ,std::vector<std::string>& result)
{
	...
	if(POSOK(trimDelimiter.find(replica[0])))
	{
		...
		if(index >= str.size())  //全部是空格或者TAB返回
			return false;
	}
	...
	while ( length > curPos )
	{
		...
		if(!bInQuotes && POSOK(trimDelimiter.find(ch))) // 去掉非引用中多余的空格或者TAB
		{
			if(curPos-1 >= 0 && POSOK(trimDelimiter.find(pStrPointer[curPos-1]))) // 前一个字符是空格或者TAB
			{
				curPos++;
				continue;
			}
			else if(curPos-1 >= 0 && !POSOK(trimDelimiter.find(pStrPointer[curPos-1]))) // 前一个字符不是空格或者TAB
			{
				if(!args.empty())
				{
		...
		if(POSOK(escape.find(ch)))  //如果是backslash
		{
			if((curPos+1 < length && POSOK(quote.find(pStrPointer[curPos+1])))) //backslash后是"
			{
				curPos = curPos + 2;
				if(bInQuotes)
					quotes.push_back('"');
