...
uint8* BufferList::bd(size_t index, size_t& len, size_t& size) const
{
	size = len =0;
	if (index <0 || index >= _bdlist.size())
		return NULL;
	len = _bdlist[index].len;
	size = _bdlist[index].size;
	return _bdlist[index].data;
}
