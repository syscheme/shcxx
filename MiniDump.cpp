...
static bool validatePath(const TCHAR *     wszPath )
{
	...
	if (!::CreateDirectory(wszPath, NULL) && RROR_ALREADY_EXISTS != ::GetLastError())
		return false;
...
MiniDump::MiniDump(TCHAR* DumpPath, TCHAR* ModuleName)
{
	...
	else _tcscpy(_moduleName, ModuleName);
	...
	else
	{
		if (!validatePath(DumpPath))
		{
			...
			trace(_T("wrong dump path[%s], taking default[%s]"), DumpPath, _dumpPath);
		}
		else
		{
...
static bool generateMiniDump(MiniDump::MINIDUMPWRITEDUMP writeDump, struct _EXCEPTION_POINTERS *pEx, DWORD dwFlag, const char* folder, const char* moduleName, ZQ::common::Log& log)
{
	...
	SYSTEMTIME st;
	GetLocalTime(&st);
	_stprintf(tszFileName, _T("%s%s_%d%02d%02dT%02d%02d%02d%s.dmp"), folder, moduleName,
	...
	else log(Log::L_ERROR, CLOGFMT("MiniDump", "Module[%s] crashed, writing crash dump to file %s"), moduleName, tszFileName);
