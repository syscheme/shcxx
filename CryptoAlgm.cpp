...
#else
#define blk0(i) (block->l[i] = (rol(block->l[i],24)&(SHA1::quadbyte)0xFF00FF00) \
		|(rol(block->l[i],8)&(SHA1::quadbyte)0x00FF00FF))
#endif
...
void SHA1::SHA1_Transform(SHA1::quadbyte state[5], SHA1::byte buffer[64]) {
	SHA1::quadbyte	a, b, c, d, e;
	BYTE64QUAD16	*block;
...
void SHA1::SHA1_Update(SHA1::CTX *context, const SHA1::byte *data, unsigned int len) {
	unsigned int	i, j;
	//To avoid modifying the original data
	//********************************************************
	//unsigned char *MicTempBuffer=(unsigned char*)malloc(len);//added by WangJS 2005.4.20
	unsigned char MicTempBuffer[1024]={0};
...
void SHA1::SHA1_Final(SHA1::byte digest[SHA1_DIGEST_LENGTH], SHA1::CTX *context) {
	quadbyte	i, j;
	byte	finalcount[8];
